...
def run(workspace: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="device-reviews-") as temporary:
        ...
        try:
            ...
            job = service.renew_job("stat-1", "worker-1", job["job_id"], job["lease_token"], lease_seconds=60)
            analysis = service.complete_job("stat-1", "worker-1", job["job_id"], job["lease_token"])
